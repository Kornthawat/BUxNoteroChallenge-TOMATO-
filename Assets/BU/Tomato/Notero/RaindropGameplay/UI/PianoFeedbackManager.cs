...
namespace Notero.TOMATO.RaindropGameplay.UI
{
    public class PianoFeedbackManager : MonoBehaviour
    {
        ...
        public virtual void Init(BaseVirtualPianoController virtualPianoController, List<float> lanePosX)
        {
            m_VirtualPianoController = virtualPianoController;
        ...
        {
            var index = m_VirtualPianoController.GetPianoKeyIndex(midiId);

            if(m_VirtualPianoController.IsMidiIdInRange(midiId) && GetPositionXByIndex(index, out var posX))
            {
