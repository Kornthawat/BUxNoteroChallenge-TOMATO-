using Notero.TOMATO.Raindrop.UI;
...
{
    public class BaseRaindropNoteController : MonoBehaviour
    {
        [SerializeField]
        protected RaindropNoteSpawner m_RaindropNoteSpawner;

        [SerializeField]
        protected RaindropBarlineSpawner m_RaindropBarlineSpawner;
        ...
        public List<RaindropBarline> RaindropBarlineList => m_RaindropBarlineSpawner.GetBarlineList();
        ...
        protected bool HasRaindropBarline() => m_RaindropBarlineSpawner.GetBarlineList()?.Count > 0;

        public virtual void BarlineSetup(float bpm, float timeSignatureTop, float timeSiganatureBottom, float songDuration)
        {
            if(m_RaindropBarlineSpawner == null) return;

            var barPeriod = m_RaindropBarlineSpawner.GetBarLinePeriod(bpm, timeSignatureTop, timeSiganatureBottom);
            var barCount = m_RaindropBarlineSpawner.GetBarLineCount(songDuration, barPeriod);
            m_RaindropBarlineSpawner.SpawnBarlines(barCount);
            m_RaindropBarlineSpawner.SetBarLineEndTime(RaindropScrollSpeed, barPeriod);
        }
        ...
        private void CheckAccidentalText(string noteName, RaindropNote note, MidiNoteInfo info)
        {
            ...
            {
                if(accidentalNumber < 0) /*have flats*/
                {
        ...
        public virtual void UpdateLogic(float currentTime)
        {
            ...
            {
                foreach(RaindropBarline barline in m_RaindropBarlineSpawner.GetBarlineList())
                {
